...
resolution = 15 # 96 used for production runs

reltol = 1e-9 # relative tolerance for integration
abstol = 1e-5 # absolute tolerance for integration
boozmn_filename = 'boozmn_qhb_100.nc' # netcdf file, 4608 modes in total
...
gc_tys, res_hits = trace_particles_boozer(
        ...
        Ekin=Ekin, stopping_criteria=[MaxToroidalFluxStoppingCriterion(0.9)],
        forget_exact_path=False, zetas_stop=False, vpars_stop=False,  mode='gc_vac',solver_options=solver_options, axis=2)
...
if (comm.rank == 0):
   ...
   np.savetxt('res_hits_cpu.txt',res_lost)
   np.save('gc_tys_cpu.npy', np.array(gc_tys, dtype=object), allow_pickle=True)
